...
# Helper to read current PowerState
...
Write-Host ("Will deallocate (stop) the following VMs:`n - {0}" -f ($vmNames -join "`n - "))
...
foreach ($vm in $vmNames) {
    ...
    if ($state -eq 'PowerState/deallocated') {
        $vmSW.Stop()
        Write-Host ("[SKIP] {0} is already deallocated. (elapsed {1})" -f $vm, (Format-Elapsed $vmSW.Elapsed)) -ForegroundColor Yellow
        continue
    }

    Invoke-AzCli -Args @('vm','deallocate','-g',$RG,'-n',$vm,'-o','none') -OkMessage "Deallocated $vm"

    $vmSW.Stop()
    Write-Host ("[TIME] VM {0} deallocated in {1}" -f $vm, (Format-Elapsed $vmSW.Elapsed)) -ForegroundColor Cyan
}
